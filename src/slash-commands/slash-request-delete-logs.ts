...
export default function Command(): WSlashCommand {
  return {
    ...
    description: 'Request deletion of logs related to you. Guaranteed deletion within two weeks.',
    ...
    execute: async (interaction) => {
      try {
        ...
        if (!messageLogs || !messageLogs.isTextBased()) {
          console.warn(
            "[slash-request-delete-logs] 'message_logs' channel does not exist, skipping"
          )
          ...
          if (staffLog?.isTextBased()) {
            ...
            staffLog.send({
              content: dev
                ? `<@&${dev.id}> The \`message_logs\` channel does not exist. User tried to request log deletion.`
                : 'The `message_logs\` channel does not exist. User tried to request log deletion. (Developer role not found)',
            })
          }
        ...
        await supaClient.from('DiscordScheduleMessageDeletion').insert([
          {
            channel_id: messageLogs.id,
            message_ids: messageIds,
            request_by_id: interaction.user.id,
            execute_at: new Date(Date.now() + 7 * 24 * 60 * 60 * 1000).toISOString(), // 1 week later
          },
        ])
        ...
        await interaction.reply({
          content: 'Something went wrong while processing your request. Please contact staff.',
          flags: 'Ephemeral',
        })
      }
    },
