    ...
    execute: async (interaction) => {
      try {
        ...
        if (cached) {
          ...
          if (!error && data) {
            const logsAfter = data.find(
              (d: { key: string; value: string }) =>
                d.key === "AUTO_DELETE_LOGS_AFTER"
            );
            const updated = data.find(
              (d: { key: string; value: string }) =>
                d.key === "AUTO_DELETE_LOGS_AFTER_LAST_TIMESTAMP"
            );
            ...
            updatedTimestamp = updated?.value
              ? new Date(updated.value).toLocaleDateString()
              : "N/A";
        ...
        const embed = new EmbedBuilder()
          .setAuthor({
            ...
            iconURL:
              "https://cdn.wikisubmission.org/media/brand-assets/logo-black.png",
          })
          .setColor(0x00bcd4)
