...
export class Bot {
  ...
  async registerAppCommands() {
    ...
    const slashCommands =
      await getFileExports<WSlashCommand>("/slash-commands");
    const userCommands = await getFileExports<WUserCommand>("/user-commands");
    const messageCommands =
      await getFileExports<WMessageCommand>("/message-commands");
    ...
    try {
      ...
      console.log(
        `Slash commands Synced: ${slashCommands.map((c) => `/${c.name}`).join(", ")}`
      );
      console.log(
        `User commands Synced: ${userCommands.map((c) => `/${c.name}`).join(", ")}`
      );
      console.log(
        `Message commands Synced: ${messageCommands.map((c) => `/${c.name}`).join(", ")}`
      );
    } catch (error) {
  ...
  async registerEventListeners() {
    ...
    const eventListeners =
      await getFileExports<WEventListener>("/event-listeners");
  ...
  async registerScheduledTasks(): Promise<void> {
    ...
    const scheduledActions =
      await getFileExports<ScheduledTaskManager>("/scheduled-tasks");
    ...
    console.log(
      `Scheduled tasks: ${scheduledActions.map((s) => s.action.id).join(", ")}`
    );
  }
    ...
    this.addEventListener("interactionCreate", async (interaction) => {
      ...
      // ---- USER CONTEXT MENU COMMANDS ----
        ...
        try {
          ...
          logError(
            error,
            `interactionCreate (User Context /${interaction.commandName})`
          );
        }
      }
      ...
      // ---- MESSAGE CONTEXT MENU COMMANDS ----
        ...
        const cmd = messageCommands.find(
          (c) => c.name === interaction.commandName
        );
        if (!cmd) {
          console.warn(
            `[${interaction.id}] No matching message command found.`
          );
          return;
        }
        ...
        try {
          ...
          logError(
            error,
            `interactionCreate (Message Context /${interaction.commandName})`
          );
        }
      }
