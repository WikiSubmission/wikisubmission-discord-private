    ...
    search: {
        parameters: {
            query?: {
                ...
                limit?: number;
                offset?: number;
                /** @description Enable hybrid semantic search (FTS + vector RRF) when the server has an embedding model configured. Defaults to true when OPENAI_API_KEY is set. Set to false to force pure keyword FTS. */
                semantic?: boolean;
                /** @description Language codes for word-level text (subset of langs, e.g. only "ar"). */
                word_langs?: components["parameters"]["WordLangsParam"];
                /** @description Include word-by-word breakdown for each verse. */
